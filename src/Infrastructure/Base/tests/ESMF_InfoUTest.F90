...
program ESMF_InfoUTest
  ...
  call ESMF_InfoDestroy(attrs3, rc)
  if (rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT)
  !----------------------------------------------------------------------------
  ...
  do i=1, 100000
    ...
    call ESMF_InfoDestroy(attrs4, rc)
    if (rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT)
  end do
  ...
  call ESMF_InfoDestroy(attrs5, rc)
  if (rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT)
  !----------------------------------------------------------------------------
  ...
  call ESMF_InfoDestroy(attrs6, rc)
  if (rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT)
  !----------------------------------------------------------------------------
...
!  if (rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT)
  ...
  call ESMF_InfoGet(attrs9, "the-key", arr_i4_get, arr_i4_get_count, rc=rc)
  if (rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT)
...
!  print *, fails_obj
  ...
  call ESMF_InfoInquire(attrs_inq, size=count, isArray=isArray, &
